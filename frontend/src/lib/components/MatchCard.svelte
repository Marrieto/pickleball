<script lang="ts">
	...
	let scoreA = $derived(match.score?.teamAPoints ?? 0);
	let scoreB = $derived(match.score?.teamBPoints ?? 0);
	...
	function onPick(team: 'A' | 'B', value: number) {
		...
		tournamentStore.recordScore(roundId, match.court, next.teamAPoints, next.teamBPoints);
	}
...
<div class="card match">
	...
	<div class="content">
		...
		<div class="team">
			<div class="team-info">
				{#if readOnly}
					{#if sideLabels.teamA}<span class="side-label">{sideLabels.teamA}</span>{/if}
				{:else if editingSide === 'teamA'}
					<input
			...
			{#if !hideScores}
			{#if readOnly}<strong>{match.score?.teamAPoints ?? '–'}</strong>{:else}
			<ScorePicker
				value={scoreA}
				max={maxScore}
		...
		<div class="team">
			<div class="team-info">
				{#if readOnly}
					{#if sideLabels.teamB}<span class="side-label">{sideLabels.teamB}</span>{/if}
				{:else if editingSide === 'teamB'}
					<input
						class="side-label-input"
						bind:value={labelDraft}
			...
			{#if !hideScores}
			{#if readOnly}<strong>{match.score?.teamBPoints ?? '–'}</strong>{:else}
			<ScorePicker
				value={scoreB}
				max={maxScore}
